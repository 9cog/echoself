...
interface ComponentRegisteredEvent {
  componentId: string;
  [key: string]: unknown;
}

interface ContentTransferEvent {
  from: string;
  to: string;
  [key: string]: unknown;
}
...
export class FeedbackIntegrationService {
  ...
  private initializeIntegration(): void {
    ...
    this.orchestratorService.addEventListener(
      ...
      (data: unknown) => {
        const eventData = data as ComponentRegisteredEvent;
        console.log(`🔗 Component registered: ${eventData.componentId}`);
    ...
    this.orchestratorService.addEventListener(
      "content_transfer",
      (data: unknown) => {
        const eventData = data as ContentTransferEvent;
        console.log(`📤 Content transfer: ${eventData.from} → ${eventData.to}`);
