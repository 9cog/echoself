...
import { EchoSpaceControlPanel } from "./EchoSpaceControlPanel";
...
const MapView: React.FC = () => {
  ...
  const [showEchoSpace, setShowEchoSpace] = useState<boolean>(false);
  ...
  const handleRoomSelect = (roomId: RoomType) => {
    ...
    // Automatically show EchoSpace when echospace is selected
    if (roomId === "echospace") {
      setShowEchoSpace(true);
    }
  };
  ...
  // Toggle EchoSpace visibility
  const toggleEchoSpace = () => {
    setShowEchoSpace(!showEchoSpace);
  };
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="flex-none h-12 bg-card text-card-foreground px-4 flex items-center justify-between border-b border-border">
        ...
        <div className="flex items-center">
          ...
          <button
            onClick={toggleEchoSpace}
            className={`p-2 rounded-md transition-colors ${
              showEchoSpace ? "text-primary" : "hover:bg-primary/20"
            }`}
            title="EchoSpace Control Panel"
          >
            <FiZap size={20} />
          </button>
      ...
      <div className="flex-1 flex overflow-hidden">
        {/* Map Panel */}
        <div
          className={`${showChat || showMemory || showDiagrams || showEchoSpace ? "w-1/2" : "w-full"} h-full transition-all duration-300`}
        >
          <EchoHomeMap onRoomSelect={handleRoomSelect} />
        </div>

        {/* Right Panel - Chat, Memory, Diagrams, or EchoSpace */}
        {(showChat || showMemory || showDiagrams || showEchoSpace) && (
          <div className="w-1/2 h-full border-l border-border">
            {showMemory && (
              <div
                className={`h-full ${showChat || showDiagrams || showEchoSpace ? "h-1/2" : "h-full"}`}
              >
                <Mem0AI />
              </div>
            )}
            ...
            {showChat && (
              <div
                className={`${showMemory || showDiagrams || showEchoSpace ? "h-1/2 border-t border-border" : "h-full"}`}
              >
                <Chat />
              </div>
            )}
            ...
            {showDiagrams && (
              <div
                className={`${showMemory || showChat || showEchoSpace ? "h-1/2 border-t border-border" : "h-full"}`}
              >
            ...
            {showEchoSpace && (
              <div
                className={`${showMemory || showChat || showDiagrams ? "h-1/2 border-t border-border" : "h-full"}`}
              >
