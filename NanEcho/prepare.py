...
def main():
    ...
    # --- Data Splitting with Size Validation ---
    print("\n📂 Splitting data into training and validation sets...")
    n_tokens = len(token_ids)
    
    # Validate total token count against typical block sizes
    MIN_BLOCK_SIZE = 256  # Minimum reasonable block size
    MAX_BLOCK_SIZE = 1024  # From our config files
    
    if n_tokens <= MAX_BLOCK_SIZE:
        print(f"⚠️  Warning: Dataset size ({n_tokens:,} tokens) is smaller than typical block_size ({MAX_BLOCK_SIZE})")
        print(f"   This may cause training errors. Consider:")
        print(f"   1. Adding more source data")
        print(f"   2. Generating more synthetic samples") 
        print(f"   3. Using block_size < {n_tokens}")
        
        if n_tokens <= MIN_BLOCK_SIZE:
            print(f"❌ Error: Dataset too small ({n_tokens:,} tokens < {MIN_BLOCK_SIZE})")
            print(f"   Cannot proceed with training. Please add more source data.")
            sys.exit(1)
    ...
    if len(train_data_ids) <= MAX_BLOCK_SIZE:
        print(f"⚠️  Warning: Training split ({len(train_data_ids):,} tokens) smaller than block_size ({MAX_BLOCK_SIZE})")
    if len(val_data_ids) <= MAX_BLOCK_SIZE:
        print(f"⚠️  Warning: Validation split ({len(val_data_ids):,} tokens) smaller than block_size ({MAX_BLOCK_SIZE})")
